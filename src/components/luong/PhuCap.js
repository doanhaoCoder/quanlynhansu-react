...
const PhuCap = () => {
  ...
  useEffect(() => {
    const fetchBangLuong = async () => {
      try {
        ...
        setBangLuong(updatedData.filter(item => item.PhuCap > 0));
      } catch (error) {
  ...
  return (
    <div className=" mt-4">
      <h2>Danh Sách Nhân Viên Có Phụ Cấp</h2>
      ...
      <table className="table table-bordered table-striped mt-3">
        <thead className="table-dark">
          <tr>
            ...
            <th onClick={() => handleSort("PhuCap")}>Lương Phụ Cấp {renderSortIcon("PhuCap")}</th>
            <th onClick={() => handleSort("GhiChuPhuCap")}>Ghi Chú Phụ Cấp {renderSortIcon("GhiChuPhuCap")}</th>
        ...
        <tbody>
          {searchedBangLuong.map((item, index) => (
            <tr key={item.id}>
              ...
              <td>{item.PhuCap.toLocaleString()} VNĐ</td>
              <td>{item.GhiChuPhuCap}</td>
